...
RCT_EXPORT_MODULE(RCTMessageListViewManager)
- (UIView *)view
{
 ...
 **/
- (void)onOpenPhotosItem{
    if(!_messageList.onAvatarClick) { return; }
  _messageList.onAvatarClick(@{@"message":@"打开相册"});
}
- (void)onOpenCameraItem{
   if(!_messageList.onMsgLongClick) { return; }
  _messageList.onMsgLongClick(@{@"message":@"打开相机"});
}

-(void)onSendText:(NSString*)message{
  if(!_messageList.onMsgClick) { return; }
  _messageList.onMsgClick(@{@"message":message});
}
